...
Cognia archives old sessions to MinIO (S3-compatible storage) for long-term retention. **Archived data can be searched via the `include_archived` parameter** - when enabled, the system searches both active data in Neo4j and archived sessions in MinIO. **Note:** Archived memory is disabled by default due to performance impact (~1000ms latency from on-the-fly embedding generation).
...
- Sessions include messages, entities, and tool calls
...
- Archived messages are retrieved using semantic similarity search
- Results from both sources are merged and reranked together
- **Disabled by default** (`include_archived=false`) due to performance impact (~1000ms latency)
...
When `recall_memories` is called with `include_archived=true` (disabled by default due to performance):
...
3. **Archived Memory Search**: 
   ...
   - Retrieves each session and generates embeddings for messages on-the-fly
...
**Performance Note**: Generating embeddings for archived messages on-the-fly adds latency. For better performance, consider storing embeddings in archived data (future enhancement).

## Future Enhancements

Potential improvements:

1. **Embedding Storage**: Store embeddings in archived JSON to avoid on-the-fly generation
2. **Selective Recall**: Allow users to specify time ranges that include archived data
3. **Caching**: Cache frequently accessed archived sessions in memory
...
```typescript
const archivalService = getArchivalService();
const sessionData = await archivalService.retrieveSession('sessions/session-123/2024-01-01T00:00:00Z.json');
...
console.log(`Restored ${stats.messageCount} messages, ${stats.entityCount} entities`);
```

**Important Notes**:
- Restoring sessions regenerates embeddings and recreates entities/facts, which may take time for large sessions
...
- Embeddings are generated in parallel before the transaction to optimize performance

### Using Archived Memory in Retrieval

Archived memory is available via the `include_archived` parameter in the `recall_memories` MCP tool. **Note:** This is disabled by default (`include_archived=false`) because it adds ~1000ms latency due to on-the-fly embedding generation. Enable it only when you need to search long-term archived data.
