...
sequenceDiagram
    ...
    loop For each old session
        ArchivalService->>ArchivalService: Build session JSON with embeddings
        Note over ArchivalService: {id, messages (with embeddings),<br/>entities, tools}
        ...
        Note over Neo4j: Session data in MinIO
    end
